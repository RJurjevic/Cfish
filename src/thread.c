...
static THREAD_FUNC thread_init(void *arg)
{
  ...
  if (settings.numaEnabled) {
...
#ifndef NNUE_PURE
    pos->pawnTable = calloc(1, PAWN_ENTRIES * sizeof(PawnEntry));
    pos->materialTable = calloc(1, 8192 * sizeof(MaterialEntry));
#endif
    ...
    pos->stackAllocation = calloc(1, 63 + (MAX_PLY + 110) * sizeof(Stack));
    pos->moveList = calloc(1, 10000 * sizeof(ExtMove));
  }
